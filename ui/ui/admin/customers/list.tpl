...
<script>
    ...
    $('#bulkDisable').click(function () {
        ...
        if (ids.length === 0) return alert('Select customer');
    
        if (!confirm('Disable selected?')) return;
    
        $.post('?_route=customers/bulk-disable', {
            customer_ids: ids,
            token: '{$csrf_token}'
        }, function () {
            location.reload();
    ...
    $('#bulkEnable').click(function () {
        ...
        if (!confirm('Enable selected?')) return;
    
        $.post('?_route=customers/bulk-enable', {
            customer_ids: ids,
            token: '{$csrf_token}'
        }, function () {
            location.reload();
    ...
    // 🔵 Recharge
    $('#bulkRecharge').click(function () {
        let ids = getSelectedIds();
        if (ids.length === 0) return alert('Select customer');
    
        // redirect with ids
        window.location.href = '?_route=plan/bulk-recharge&ids=' + ids.join(',');
    });
