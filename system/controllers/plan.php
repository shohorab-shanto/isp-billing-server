...
switch ($action) {
        case 'bulk-recharge':
                ...
                $ui->display('admin/plan/bulk-recharge.tpl');
            break;
        case 'bulk-recharge-post':
        
            // ✅ CSRF protection
            if (!Csrf::check($_POST['csrf_token'])) {
                r2(getUrl('customers/list'), 'e', 'Invalid Token');
            }
        
            // ✅ Convert IDs string → array
            $ids = explode(',', $_POST['ids']);
        
            $plan_id = $_POST['plan_id'];
            $server = $_POST['router_id'];
        
            // ✅ Validate input
            if (empty($ids) || empty($plan_id) || empty($server)) {
                r2(getUrl('customers/list'), 'e', 'All fields are required');
            }
        
            $plan = ORM::for_table('tbl_plans')->find_one($plan_id);
        
            if (!$plan) {
                r2(getUrl('customers/list'), 'e', 'Plan not found');
            }
            ...
            r2(
                getUrl('customers/list'),
                's',
                "Bulk Recharge Completed: $success success, $failed failed"
            );
        break;
        case 'sync':
    ...
    case 'print-voucher':
        ...
        $ui->assign('ids', $ids);
        $ui->assign('plans', $plans);
        $ui->assign('routers', $routers); // ✅ ADD THIS
    ...
    if (!Csrf::check(_req('token'))) {
        r2(getUrl('customers'), 'e', 'Invalid Token');
    }

    foreach ($ids as $customer_id) {

        $c = User::_info($customer_id);
        ...
        if ($c && $p) {
            ...
            _log('Bulk Recharge ' . $c['username'], 'User', $customer_id);
        }
    }

    r2(getUrl('customers'), 's', 'Bulk Recharge Success');
    break;
